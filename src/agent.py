...
# Fetch ADC (Application Default Credentials), used as agent's identity to access BigQuery MCP Server
...
if not project_id:
  raise ValueError("[ERROR] GOOGLE_CLOUD_PROJECT environment variable is not set.")

# Builds authentication headers for MCP Server requests, refresh creds
...
# Initialize MCP toolset with the conn parameters
bigquery_toolset = McpToolset(
  connection_params=StreamableHTTPConnectionParams(
    ...
    tool_filter=[
      ...
      'list_table_ids',
      'get_table_info',
      'execute_sql_readonly' # prevent accidental modifications
    ]
  ),
